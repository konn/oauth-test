...
mkYesodDispatch "Testing" resourcesTesting
...
withTesting :: AppConfig -> Logger -> (Application -> IO a) -> IO ()
withTesting conf logger f = do
#ifdef PRODUCTION
...
withDevelAppPort =
  ...
  where
    ...
    go f = do
        ...
        withTesting conf logger $ \app -> f (port, debugHandle (logHandle logger) app)
        flushLogger logger
      where
